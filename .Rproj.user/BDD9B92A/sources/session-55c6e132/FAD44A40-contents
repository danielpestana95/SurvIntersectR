#' Find Intersection Points of Survival Curves
#'
...
#' @return A data frame containing the coordinates of the intersection points, or NULL if there are no intersections.
#' @examples
#' \dontrun{
#'   library(survminer)
#'   library(survival)
...
utils::globalVariables(c("%>%", "mutate", "x", "x_int", "y1", "y2"))

find_surv_intersect <- function(ggsurvplot_object) {
  ...
  group_2_clean_surv_data <- unique(group_2_clean_surv_data) %>%
    ...
    dplyr::summarize(y2 = min(y2, na.rm = TRUE))
  ...
  group_1_clean_surv_data <- unique(group_1_clean_surv_data)%>%
    dplyr::group_by(x_int) %>%
    dplyr::summarize(y1 = min(y1, na.rm = TRUE))
  ...
  all_groups_clean_surv_data = tidyr::drop_na(all_groups_clean_surv_data)

#Check Where the sign change
diff_signs <- diff(all_groups_clean_surv_data$sub)

# Identify indices where the sign changes
sign_change_indices <- which(diff_signs != 0)

x_where_sign_changes = all_groups_clean_surv_data[sign_change_indices, ] %>%
  mutate(x = x_int/10^maximum_decimal_places_in_both_curves) %>%
  dplyr::select(x)
